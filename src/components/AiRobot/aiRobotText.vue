    ...
    <div class="flex w-full h-full pt-14">
      ...
      <div class="flex-none w-0 md:w-[260px] bg-slate-200 border-r-2 hidden md:block p-8 text-sm">
        ...
        <t-card class="bg-orange-100 mt-8 text-gray-600 text-sm" v-if="sdServerType === 'common'">
          ...
          建议使用
          <t-tooltip
            content='购买后上面服务器地址填写 工作台-小羊驼 域名，同时把端口换成8000，并添加"/v1"后缀'
            theme="primary"
          >
            <t-link
              href="https://inscode.csdn.net/gpu?utm_source=sd_app"
              target="_blank"
              :suffix-icon="renderSuffixIcon"
              underline
              >私有服务器</t-link
            >
          </t-tooltip>
        ...
        >
          <t-form-item label="机器人头像">
            ...
            <t-space
              direction="vertical"
              align="center"
              v-for="imgItem in robotAvatarList"
          ...
          <t-form-item label="你的头像">
            <t-space
              direction="vertical"
              align="center"
              v-for="imgItem in userAvatarList"
              class="mr-2 cursor-pointer hover:bg-blue-600"
            >
              <t-image
                class="rounded-lg border-2"
        ...
        <div class="w-full bg-slate-200 h-full m-auto relative container max-w-6xl rounded-xl">
            ...
            <div v-for="(item, index) in message" class="mb-8">
              <div v-if="item.user === 'User'">
                <div class="flex flex-row-reverse">
                  ...
                  ></t-image>
                  <div
                    class="bg-green-400 text-gray-700 p-4 mx-2 w-fit max-w-2xl 2xl:max-w-4xl rounded-lg cursor-pointer text-sm"
          ...
          >
            <div class="w-full h-10 flex">
              <input
                v-model="prompt"
                class="flex-auto ring-0 outline-0 pl-2 bg-slate-200 h-10 border-0"
            ...
            <div class="w-full h-10 mt-2 flex gap-2" v-if="mode === 'draw'">
              ...
              <div class="flex-none w-40 ring-0 outline-0 pl-2 h-10 flex justify-between">
                <input
                  v-model="width"
                  class="ring-0 outline-0 pl-2 bg-slate-200 h-10 w-16"
                  placeholder="width"
                />
              ...
              <input
                v-model="steps"
                class="flex-none w-30 ring-0 outline-0 pl-2 bg-slate-200 h-10 w-16"
                placeholder="steps"
              />
              <input
                v-model="sampler"
