...
export default {
  ...
  set(tableName = defaultTableName, val, id = 100) {
    let request = db.transaction(tableName, 'readwrite').objectStore(tableName).add({
      id: id,
      data: val,
    });
    return new Promise((resolve, reject) => {
  ...
  put(tableName = defaultTableName, val, id = 100) {
    let request = db.transaction(tableName, 'readwrite').objectStore(tableName).put({
      id: id,
      data: val,
    });
    return new Promise((resolve, reject) => {
  ...
  get(tableName = defaultTableName, id = 100) {
    let request = db.transaction(tableName, 'readwrite').objectStore(tableName).get(id);
